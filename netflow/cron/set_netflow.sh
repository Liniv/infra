...
nfdump -R /data/$(date +%Y)/$(date +%m)/$(date +%d)/nfcapd.$(date +'%Y%m%d%H%M' -d "now -1 minutes") -N -o fmt:%ts,%sa,%da,%byt -q >> /data/netflow.csv
chmod 777 /data/netflow.csv
